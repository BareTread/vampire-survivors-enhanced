export class BaseWeapon {
    ...
    shouldFire() {
        ...
        const enemies = this.game.systems.enemy.getEnemiesInRange(this.player.x, this.player.y, this.targetingRange);
    ...
    createMuzzleFlash() {
        ...
        if (this.game.systems.particle) {
            this.game.systems.particle.createBurst(this.player.x, this.player.y, 'weaponFire', {
                color: flashColor,
    ...
    updateStats() {
        ...
        const projectileCount = this.baseStats.projectiles + playerStats.projectiles;

        this.currentStats = {
            damage: this.baseStats.damage * playerStats.damage,
            ...
            projectiles: Math.max(0, Math.min(projectileCount, this.maxProjectiles)),
            piercing: this.baseStats.piercing
        };
    }

    getEffectiveCooldown() {
        return Math.max(this.minCooldown, this.currentStats.cooldown);
    }
    ...
    createEnhancedHitEffects(enemy, damage, critical, wasManualAim = false, accuracyBonus = 1.0, combo = 1) {
        ...
        if (this.game.systems.particle.createEnhancedHitEffect) {
            ...
            this.game.systems.particle.createEnhancedHitEffect(enemy.x, enemy.y, damage, this.type, critical, options);
        } else {
            // Fallback to original method
        ...
        if (wasManualAim && accuracyBonus > 2.0 && this.game.systems.particle.createSkillShotEffect) {
            this.game.systems.particle.createSkillShotEffect(enemy.x, enemy.y, accuracyBonus);
        }
    }
    ...
    createHitEffects(enemy, damage, critical, wasManualAim = false, accuracyBonus = 1.0) {
        ...
        if (this.game.systems.particle.createEnhancedBurst) {
            this.game.systems.particle.createEnhancedBurst(enemy.x, enemy.y, effectType, {
                color: this.getHitEffectColor(critical, wasManualAim),
                intensity: intensity,
        ...
        if (wasManualAim && accuracyBonus > 2.0 && this.game.systems.particle.createPrecisionRing) {
            this.game.systems.particle.createPrecisionRing(enemy.x, enemy.y, '#00FFFF', accuracyBonus);
        }
    ...
    playHitSound(damage, critical, wasManualAim = false) {
        ...
        const pitch = critical ? 0.8 : wasManualAim ? 1.2 : 1.0 + (Math.random() - 0.5) * 0.2;
    ...
    getBloodColor(enemy) {
        ...
        switch (enemy.type) {
            case 'undead':
                return '#4A0080';
            case 'demon':
                return '#4B0082'; // Changed from red to indigo
            case 'beast':
    ...
    rewardSkillfulAiming(enemy, accuracyBonus) {
        ...
        this.player.addDamageNumber(`SKILL x${accuracyBonus.toFixed(1)}!`, '#00FFFF', 'PRECISION');
