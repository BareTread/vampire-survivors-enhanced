...
export class ProgressionTelemetry {
    ...
    calculatePlayerDPS() {
        ...
        for (const weapon of player.weapons.values()) {
            ...
            const weaponDPS =
                (weapon.getEffectiveDamage() * (weapon.currentStats.projectiles + 1)) / weapon.getEffectiveCooldown();
            totalDPS += weaponDPS;
        }
    ...
    takeSample() {
        ...
        if (this.game.showDebug && this.currentMetrics.gameTime % 60 < this.sampleInterval) {
            console.log(
                `📊 Progression: ${Math.floor(this.currentMetrics.gameTime / 60)}min | Level ${sample.level} | DPS ${sample.dps.toFixed(0)} | Kill Rate ${sample.killRate.toFixed(1)}/s | Difficulty ${sample.difficulty.toFixed(1)}x`
            );
        }
    }
    ...
    analyzeProgressionIssues(sample) {
        ...
        this.detectedIssues = this.detectedIssues.filter((issue) => sample.timestamp - issue.timestamp < 300);
    }
    ...
    render(ctx) {
        if (!this.enabled || !this.game.showDebug) return;
